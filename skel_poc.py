...
from hamner_bridge import prepare_hamner, hamner_status, load_hamner_bodies, build_hamner_fullbody_frame
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.21.3-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.21.3 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.21.3 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        _atlas_status=bodyparts3d_atlas_status()
        if not _atlas_status.get("ready"):
            st.warning("V110.3.21.3 · Atlas BodyParts3D real todavía no está instalado en esta instancia.")
            st.caption("Se descarga a caché temporal runtime. No se guarda en Supabase ni dentro del NPZ.")
            if st.button("🦴💪 Preparar atlas anatómico real BodyParts3D",key="v2017_install_bp3d",use_container_width=True):
                with st.spinner("Descargando y preparando huesos/músculos anatómicos reales…"):
                    try:
                        _r=ensure_bodyparts3d_atlas(force=False)
                        st.session_state["v2017_atlas_install_result"]=_r
                        if _r.get("ok"):
                            st.success(f"Atlas preparado · {_r.get('bones',0)} huesos · {_r.get('muscles',0)} músculos · {_r.get('downloaded',0)} descargados ahora · {_r.get('cached',0)} en caché.")
                            st.rerun()
                        else:
                            st.error("No se pudo preparar ninguna malla BodyParts3D.")
                            if _r.get("missing"): st.code("\n".join(_r["missing"][:12]))
                    except Exception as _e:
                        st.error(f"Fallo al preparar BodyParts3D: {type(_e).__name__}: {_e}")
        _atlas_status=bodyparts3d_atlas_status()

        # V110.3.21.3 · visor anatómico incremental.
        # No enviamos secuencias anatómicas completas al navegador.
        # Se mantiene UNA malla base ligera por estructura + transformaciones,
        # y se dibuja únicamente el frame seleccionado.
        _layer_choice=st.radio(
            "Capa anatómica",
            ["Piel","Esqueleto anatómico real","Piel + esqueleto"],
            horizontal=True,
            key="v2018_layer_choice"
        )

        _frame_count=int(Vviz.shape[0])
        _frame_idx=st.slider(
            "Frame anatómico",
            min_value=1,
            max_value=_frame_count,
            value=min(int(st.session_state.get("v2018_frame_idx",1)),_frame_count),
            step=1,
            key="v2018_frame_idx"
        )-1

        _groups=()
        if _layer_choice in ("Esqueleto anatómico real","Piel + esqueleto"):
            _groups=("bones",)

        atlas={"bones":[],"muscles":[],"missing":[]}
        browser_bones=[]; browser_muscles=[]
        if _groups and _atlas_status.get("ready"):
            with st.spinner("Preparando capa anatómica solicitada…"):
                atlas=_v2019_rigid_skeleton_for_mesh(mesh_seq)

                # Browser LOD only. Real BodyParts3D bones remain rigid and are not stretched.
                browser_bones=_v2018_prepare_browser_parts(atlas.get("bones",[]),max_faces_each=1800)
                browser_muscles=[]

            _vf=sum(x["browser_vertices"] for x in browser_bones+browser_muscles)
            _ff=sum(x["browser_faces"] for x in browser_bones+browser_muscles)
            st.success(
                f"✅ Esqueleto preparado · {len(browser_bones)} huesos BodyParts3D · frame {_frame_idx+1}/{_frame_count}"
            )
            st.caption(
                f"Visor ligero: {_vf:,} vértices y {_ff:,} caras. Registro global del atlas + movimiento rígido por hueso; sin estiramiento frame a frame."
            )
        elif _groups and not _atlas_status.get("ready"):
            st.info("Prepara primero el atlas BodyParts3D para habilitar el esqueleto anatómico.")

        mins=np.nanmin(Vviz.reshape(-1,3),axis=0); maxs=np.nanmax(Vviz.reshape(-1,3),axis=0)
        span=np.maximum(maxs-mins,1e-3); pad=np.maximum(.08*span,.03)
        ranges=[[float(mins[d]-pad[d]),float(maxs[d]+pad[d])] for d in range(3)]

        # IMPORTANT: only ONE skin frame is sent to browser, not 75.
        _skin_frame=np.asarray(Vviz[_frame_idx],dtype=np.float32)

        # Apply anatomical transform server-side for ONLY the selected frame.
        def _one_frame_payload(parts):
            zz=[]
            for p in parts:
                VV=(np.asarray(p["T"][_frame_idx],dtype=np.float32) +
                    (np.asarray(p["V0"],dtype=np.float32)*float(p["S"][_frame_idx])) @
                    np.asarray(p["R"][_frame_idx],dtype=np.float32).T)
                zz.append({
                    "id":p["id"],
                    "V":np.round(VV,5).tolist(),
                    "F":np.asarray(p["F"],dtype=np.int32).tolist(),
                })
            return zz

        P={
            "V":np.round(_skin_frame,5).tolist(),
            "F":F[:,:3].tolist(),
            "frame":int(_frame_idx+1),
            "nframes":int(_frame_count),
            "ranges":ranges,
            "layer":_layer_choice,
            "bones":_one_frame_payload(browser_bones),
            "muscles":[],
        }
        payload=_json.dumps(P,separators=(",",":")); js=get_plotlyjs()
        html=f"""<!doctype html><html><head><meta charset='utf-8'>
        <style>
        html,body{{margin:0;background:white;font-family:system-ui}}
        #meta{{padding:6px 10px;font-weight:600}}
        #plot{{height:710px;width:100%}}
        </style><script>{js}</script></head><body>
        <div id='meta'>Frame {int(_frame_idx+1)}/{int(_frame_count)} · {_layer_choice}</div>
        <div id='plot'></div>
        <script>
        const P={payload};
        const gd=document.getElementById('plot');
        function xyz1(q){{return [q.map(p=>p[0]),q.map(p=>p[1]),q.map(p=>p[2])]}}
        let s=xyz1(P.V), sf=P.F.map(x=>x[0]), sg=P.F.map(x=>x[1]), sh=P.F.map(x=>x[2]);
        const showSkin=(P.layer==='Piel'||P.layer==='Piel + esqueleto');
        const data=[{{
          type:'mesh3d',x:s[0],y:s[1],z:s[2],i:sf,j:sg,k:sh,
          name:'Piel SKEL',opacity:(P.layer==='Piel + esqueleto'?0.18:0.88),visible:showSkin,flatshading:false
        }}];
        for(const part of P.bones){{
          let q=xyz1(part.V),f=part.F;
          data.push({{
            type:'mesh3d',x:q[0],y:q[1],z:q[2],
            i:f.map(x=>x[0]),j:f.map(x=>x[1]),k:f.map(x=>x[2]),
            name:part.id,opacity:1,visible:true,flatshading:false
          }});
        }}
        for(const part of P.muscles){{
          let q=xyz1(part.V),f=part.F;
          data.push({{
            type:'mesh3d',x:q[0],y:q[1],z:q[2],
            i:f.map(x=>x[0]),j:f.map(x=>x[1]),k:f.map(x=>x[2]),
            name:part.id,opacity:0.72,visible:true,flatshading:false
          }});
        }}
        Plotly.newPlot(gd,data,{{
          title:'V110.3.21.3 · Anatomical Atlas Mesh',
        ...
        st.markdown("---")
        st.markdown("---")
        st.markdown("### 🦴 Esqueleto interno SKEL · piel + joints")
        st.caption(
            "V110.3.21.3 usa directamente la superficie SKEL y los joints ya calculados. "
            "No descarga ni registra un atlas para colocar el esqueleto. "
            "OpenSim Hamner permanece debajo como módulo biomecánico opcional."
        )
        _if=int(st.slider(
            "Frame · esqueleto interno",
            min_value=1,max_value=int(len(Vviz)),
            value=min(int(st.session_state.get("v3213_internal_frame",1)),int(len(Vviz))),
            key="v3213_internal_frame"
        ))-1
        _show_skin=st.checkbox("Mostrar piel SKEL semitransparente",True,key="v3213_show_skin")
        _show_centres=st.checkbox("Mostrar centros articulares",False,key="v3213_show_centres")
        try:
            _Ji=np.asarray(mesh_seq.get("joints"),dtype=np.float32)
            _JNi=[str(x) for x in mesh_seq.get("joint_names",[])]
            _parts=build_internal_skeleton(np.asarray(Vviz[_if],dtype=np.float32),_Ji[_if],_JNi)
            if _show_centres:
                _parts += joint_centres(_Ji[_if],_JNi)

            import plotly.graph_objects as _go
            _fg=_go.Figure()
            if _show_skin:
                _fv=np.asarray(Vviz[_if],dtype=np.float32)
                _ff=np.asarray(F[:,:3],dtype=np.int32)
                _step=max(1,int(np.ceil(len(_ff)/4500.0)))
                _ff=_ff[::_step]
                _fg.add_trace(_go.Mesh3d(
                    x=_fv[:,0],y=_fv[:,1],z=_fv[:,2],
                    i=_ff[:,0],j=_ff[:,1],k=_ff[:,2],
                    name="Piel SKEL",opacity=0.16,flatshading=False,showscale=False
                ))
            for _pp in _parts:
                _vv=np.asarray(_pp["V"]); _ff=np.asarray(_pp["F"])
                _fg.add_trace(_go.Mesh3d(
                    x=_vv[:,0],y=_vv[:,1],z=_vv[:,2],
                    i=_ff[:,0],j=_ff[:,1],k=_ff[:,2],
                    name=_pp["id"],opacity=0.68 if _pp.get("kind")=="joint" else 1.0,
                    flatshading=False,showscale=False
                ))
            _fg.update_layout(
                title=f"Esqueleto interno SKEL · frame {_if+1}/{len(Vviz)}",
                height=780,margin=dict(l=0,r=0,t=45,b=0),
                scene=dict(aspectmode="data")
            )
            st.plotly_chart(_fg,use_container_width=True,key="v3213_internal_skeleton_plot")

            _rows=segment_lengths(_Ji[_if],_JNi)
            if _rows:
                st.markdown("#### Longitudes segmentarias derivadas de joints SKEL")
                st.dataframe(pd.DataFrame(_rows).round(4),use_container_width=True,hide_index=True)

            st.success(
                f"Esqueleto interno generado directamente desde SKEL · frame {_if+1}/{len(Vviz)} · "
                "sin recalcular fitting ni descargar atlas."
            )
            st.info(
                "Es un esqueleto interno genérico alineado biomecánicamente con los joints SKEL. "
                "No representa la morfología ósea individual de una TC/RM."
            )
        except Exception as _e:
            st.warning(f"No se pudo construir el esqueleto interno SKEL: {type(_e).__name__}: {_e}")

        st.markdown("### 🧬 OpenSim · cuerpo completo Hamner 2010")
        st.caption(
            "V110.3.21.3 sustituye Rajagopal2016 como visor principal por FullBodyModel_Hamner2010_v2_0, "
            "que incluye miembros inferiores, tronco/cabeza y brazos simples. Las mallas de cada Body se fusionan "
            "antes del registro para evitar fragmentos dispersos."
        )
        _hs=hamner_status(); _h1,_h2,_h3=st.columns(3)
        _h1.metric("Modelo",_hs.get("model","No preparado") or "No preparado")
        _h2.metric("Geometrías",int(_hs.get("present",0))); _h3.metric("Referenciadas",int(_hs.get("mesh_count",0)))
        if st.button("🧬 Preparar OpenSim Hamner cuerpo completo",key="v3212_prepare_hamner",use_container_width=True):
            with st.spinner("Descargando modelo Hamner y sus geometrías oficiales…"):
                _hr=prepare_hamner(force=False)
            if _hr.get("ok"):
                st.success(f"Hamner preparado · {_hr.get('mesh_count',0)} geometrías · {_hr.get('downloaded',0)} descargadas · {_hr.get('cached',0)} en caché · {_hr.get('failed',0)} fallos.")
                st.rerun()
            else: st.error(f"No se pudo preparar Hamner: {_hr.get('error','error desconocido')}")
        _hs=hamner_status()
        if _hs.get("ready"):
            _hf=st.slider("Frame OpenSim cuerpo completo",1,int(len(Vviz)),min(int(st.session_state.get("v3212_frame",1)),int(len(Vviz))),key="v3212_frame")
            if st.button("🦴 Construir vista OpenSim · cuerpo completo",key="v3212_view_full",use_container_width=True):
                with st.spinner("Fusionando geometrías por Body y registrando cuerpo completo…"):
                    _hb=load_hamner_bodies(); _hp=build_hamner_fullbody_frame(_hb.get("bodies",{}),np.asarray(mesh_seq.get("joints"),dtype=np.float32),[str(x) for x in mesh_seq.get("joint_names",[])],frame=int(_hf)-1)
                st.session_state["v3212_hamner_parts"]=_hp; st.session_state["v3212_hamner_unsupported"]=_hb.get("unsupported",[])
            _hp=st.session_state.get("v3212_hamner_parts",[])
            if _hp:
                import plotly.graph_objects as _go
                _fig=_go.Figure()
                for _pp in _hp:
                    _vv=np.asarray(_pp["V"]); _ff=np.asarray(_pp["F"]); _step=max(1,int(np.ceil(len(_ff)/2200.0))); _fff=_ff[::_step,:3]
                    _fig.add_trace(_go.Mesh3d(x=_vv[:,0],y=_vv[:,1],z=_vv[:,2],i=_fff[:,0],j=_fff[:,1],k=_fff[:,2],name=_pp["body"],opacity=1.0,flatshading=False,showscale=False))
                _fig.update_layout(title=f"OpenSim Hamner2010 · cuerpo completo · frame {_hf}/{len(Vviz)}",height=780,margin=dict(l=0,r=0,t=45,b=0),scene=dict(aspectmode="data"))
                st.plotly_chart(_fig,use_container_width=True,key="v3212_hamner_full_fig")
                _u=st.session_state.get("v3212_hamner_unsupported",[])
                if _u: st.warning(f"{len(_u)} geometrías VTP no legibles; no se sustituyen por geometría inventada.")
                st.info("Vista de geometrías OpenSim fusionadas por Body y registradas sobre joints SKEL. Todavía no ejecuta IK OpenSim ni representa anatomía individual CT/MRI.")
        with st.expander("⚙️ Rajagopal2016 · rama anterior (solo referencia)",expanded=False):
            st.markdown("### 🧬 OpenSim · puente musculoesquelético experimental")
            st.caption(
                "V110.3.21.3 inicia una rama biomecánica basada en Rajagopal2016/OpenSim. "
                "No se presenta como atlas CT/MRI de alta resolución: la ventaja es conservar "
                "un modelo musculoesquelético con cuerpos, articulaciones y musculotendones definidos."
            )
            _os_status=rajagopal_status()
            _c1,_c2,_c3=st.columns(3)
            _c1.metric("Modelo","Rajagopal2016" if _os_status.get("ready") else "No preparado")
            _c2.metric("Geometrías",int(_os_status.get("present",0)))
            _c3.metric("Referenciadas",int(_os_status.get("mesh_count",0)))
            if st.button("🧬 Preparar modelo OpenSim Rajagopal2016",key="v3210_prepare_opensim",use_container_width=True):
                with st.spinner("Descargando modelo .osim y geometrías oficiales desde las rutas Rajagopal/Geometry y Geometry…"):
                    _rr=prepare_rajagopal(force=False)
                if _rr.get("ok"):
                    st.success(
                        f"OpenSim preparado · {_rr.get('mesh_count',0)} geometrías referenciadas · "
                        f"{_rr.get('downloaded',0)} descargadas · {_rr.get('cached',0)} en caché · "
                        f"{_rr.get('failed',0)} fallos. La app intenta automáticamente las dos ubicaciones oficiales de geometría del repositorio."
                    )
                    st.rerun()
                else:
                    st.error(f"No se pudo preparar OpenSim: {_rr.get('error','error desconocido')}")
            _os_status=rajagopal_status()
            if _os_status.get("ready"):
                if st.button("🦴 Vista OpenSim · miembro inferior",key="v3210_view_lower",use_container_width=True):
                    with st.spinner("Leyendo geometrías OpenSim y registrando el frame actual…"):
                        _osgeo=load_rajagopal_lower_body()
                        _frame_os=max(0,min(int(st.session_state.get("v2018_frame_idx",1))-1, len(Vviz)-1))
                        _osparts=build_rajagopal_skel_frame(
                            _osgeo.get("parts",[]),
                            np.asarray(mesh_seq.get("joints"),dtype=np.float32),
                            [str(x) for x in mesh_seq.get("joint_names",[])],
                            frame=_frame_os
                        )
                    st.session_state["v3210_opensim_parts"]=_osparts
                    st.session_state["v3210_opensim_unsupported"]=_osgeo.get("unsupported",[])
                _osparts=st.session_state.get("v3210_opensim_parts",[])
                if _osparts:
                    import plotly.graph_objects as _go
                    _fig=_go.Figure()
                    for _pp in _osparts:
                        _vv=np.asarray(_pp["V"]); _ff=np.asarray(_pp["F"])
                        _step=max(1,int(np.ceil(len(_ff)/1800.0)))
                        _fff=_ff[::_step,:3]
                        _fig.add_trace(_go.Mesh3d(
                            x=_vv[:,0],y=_vv[:,1],z=_vv[:,2],
                            i=_fff[:,0],j=_fff[:,1],k=_fff[:,2],
                            name=_pp["body"],opacity=1.0,flatshading=False,
                            showscale=False
                        ))
                    _fig.update_layout(
                        title="OpenSim Rajagopal2016 · miembro inferior · registro visual corregido",
                        height=720,margin=dict(l=0,r=0,t=45,b=0),
                        scene=dict(aspectmode="data")
                    )
                    st.plotly_chart(_fig,use_container_width=True,key="v3210_opensim_lower_fig")
                    _uns=st.session_state.get("v3210_opensim_unsupported",[])
                    if _uns:
                        st.warning(
                            f"{len(_uns)} geometrías VTP no pudieron leerse con el parser ASCII ligero. "
                            "No se sustituyen por geometría inventada."
                        )
                    st.info(
                        "Esta capa es un puente biomecánico OpenSim, no una reconstrucción ósea individual. "
                        "El escalado es estático en el frame de referencia y el movimiento posterior debe mantenerse rígido."
                    )

        # V110.3.21.3: legacy BodyParts3D video controls must not crash the OpenSim branch.
        bone_parts = locals().get("bone_parts", [])
        muscle_parts = locals().get("muscle_parts", [])
        st.markdown("#### Descargar vídeo SKEL · render offline completo")
        st.caption("Genera el MP4 desde los 75 frames ya calculados. No reproduce/captura el navegador y no vuelve a ejecutar el fitting SKEL.")
        _vc1,_vc2,_vc3,_vc4=st.columns(4)
        with _vc1:
            if st.button("🎬 Generar vídeo Piel",key="v2013_video_skin",use_container_width=True):
                try:
                    _pb=st.progress(0,text="Preparando vídeo Piel…")
                    _bytes=_v2013_render_video_mp4(mesh_seq,"skin",25)
                    st.session_state["v2013_video_skin_bytes"]=_bytes
                    _pb.progress(100,text="✓ Vídeo Piel completo")
                except Exception as _e:
                    st.error(f"No se pudo generar el vídeo Piel: {_e}")
            if st.session_state.get("v2013_video_skin_bytes"):
                st.download_button("⬇️ MP4 Piel",st.session_state["v2013_video_skin_bytes"],
                                   "PhysioSentinel_SKEL_piel_75frames.mp4","video/mp4",
                                   key="v2013_dl_skin",use_container_width=True)
        with _vc2:
            _bone_ok=bool(bone_parts)
            if st.button("🎬 Generar vídeo Huesos",key="v2013_video_bones",use_container_width=True,disabled=not _bone_ok):
                try:
                    _pb=st.progress(0,text="Preparando vídeo Huesos…")
                    _bytes=_v2013_render_video_mp4(mesh_seq,"bones",25)
                    st.session_state["v2013_video_bones_bytes"]=_bytes
                    _pb.progress(100,text="✓ Vídeo Huesos completo")
                except Exception as _e:
                    st.error(f"No se pudo generar el vídeo Huesos: {_e}")
            if st.session_state.get("v2013_video_bones_bytes"):
                st.download_button("⬇️ MP4 Huesos",st.session_state["v2013_video_bones_bytes"],
                                   "PhysioSentinel_SKEL_huesos_75frames.mp4","video/mp4",
                                   key="v2013_dl_bones",use_container_width=True)
        with _vc3:
            _muscle_ok=bool(muscle_parts)
            if st.button("🎬 Generar vídeo Músculos",key="v2013_video_muscles",use_container_width=True,disabled=not _muscle_ok):
                try:
                    _pb=st.progress(0,text="Preparando vídeo Músculos…")
                    _bytes=_v2013_render_video_mp4(mesh_seq,"muscles",25)
                    st.session_state["v2013_video_muscles_bytes"]=_bytes
                    _pb.progress(100,text="✓ Vídeo Músculos completo")
                except Exception as _e:
                    st.error(f"No se pudo generar el vídeo Músculos: {_e}")
            if st.session_state.get("v2013_video_muscles_bytes"):
                st.download_button("⬇️ MP4 Músculos",st.session_state["v2013_video_muscles_bytes"],
                                   "PhysioSentinel_SKEL_musculos_75frames.mp4","video/mp4",
                                   key="v2013_dl_muscles",use_container_width=True)
        with _vc4:
            _all_ok=bool(bone_parts or muscle_parts)
            if st.button("🎬 Generar vídeo Todas",key="v2013_video_all",use_container_width=True,disabled=not _all_ok):
                try:
                    _pb=st.progress(0,text="Preparando vídeo combinado…")
                    _bytes=_v2013_render_video_mp4(mesh_seq,"all",25)
                    st.session_state["v2013_video_all_bytes"]=_bytes
                    _pb.progress(100,text="✓ Vídeo combinado completo")
                except Exception as _e:
                    st.error(f"No se pudo generar el vídeo combinado: {_e}")
            if st.session_state.get("v2013_video_all_bytes"):
                st.download_button("⬇️ MP4 Todas",st.session_state["v2013_video_all_bytes"],
                                   "PhysioSentinel_SKEL_piel_huesos_musculos_75frames.mp4","video/mp4",
                                   key="v2013_dl_all",use_container_width=True)

        if not _atlas_status.get("ready"):
            st.info("Prepara el atlas BodyParts3D para habilitar Huesos/Músculos. La Piel puede exportarse desde el NPZ.")
        else:
            st.caption("V110.3.21.3 · Render anatómico bajo demanda: sólo se carga la capa seleccionada para reducir memoria.")
    except Exception as exc:
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.21.3 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.21.3 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
